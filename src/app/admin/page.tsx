...
const GameAnalyticsDashboard = () => {
  ...
  const gameCategoryOptions: ApexOptions = {
    ...
    colors: ["#22e3ff", "#ff37d6", "#8b3bff", "#ffb43b", "#9bf03b"],
    dataLabels: {
      enabled: true,
  ...
  return (
    <div className="space-y-8 p-6 md:p-8 bg-background text-foreground">
      <div>
        <h1 className="text-4xl font-bold text-foreground font-display text-glow-cyan">Analytics Dashboard</h1>
        <p className="text-muted-foreground mt-2">Monitor your game performance and metrics</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {quickStats.map((stat, i) => (
          <QuickStatsCard key={stat.title} stat={stat} index={i} />
        ))}
      </div>
      ...
      <div className="bg-card border border-border rounded-lg p-6 shadow-[0_0_24px_oklch(0.55_0.25_290/0.15)]">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Link href="/admin/manage-games">
            <Button variant="outline" className="border-border text-foreground hover:bg-muted hover:border-primary/50 bg-transparent">
              Manage Games
            </Button>
          </Link>
        </div>
