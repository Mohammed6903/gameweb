      ...
      <div className="min-h-screen bg-background text-foreground">
        <div className="max-w-7xl mx-auto p-6 md:p-8 lg:p-12 space-y-12">
          ...
          <Suspense fallback={<WelcomeSkeleton />}>
              <section className="relative overflow-hidden bg-card border border-border rounded-xl p-6 md:p-8 shadow-lg transition-transform hover:shadow-xl duration-300 shadow-[0_0_40px_oklch(0.55_0.25_290/0.2)]">
                <div className="absolute inset-0 pointer-events-none bg-gradient-to-br from-[oklch(0.55_0.25_290/0.35)] via-accent/15 to-transparent" />
                <div className="relative flex flex-col sm:flex-row justify-between items-center gap-6">
                  <div className="flex items-center gap-4 sm:gap-6">
                    <div className="w-14 h-14 rounded-xl bg-primary/15 border border-primary/30 flex items-center justify-center shadow-md transform transition-transform hover:scale-105 duration-300 shadow-[0_0_16px_oklch(var(--primary)/0.4)]">
                      <Gamepad2 className="w-8 h-8 text-primary" />
                    </div>
                    <div>
                      <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-foreground tracking-tight font-display text-glow-cyan">
                  ...
                  <div className="flex flex-col sm:flex-row flex-wrap items-center justify-center gap-3 sm:gap-4 mt-4 sm:mt-0">
                    <div className="flex items-center gap-2 rounded-lg bg-card/70 border border-border px-4 py-2 hover:border-primary/50 transition-colors text-foreground">
          ...
          {/* Featured Games */}
          <section>
            <h2 className="text-2xl sm:text-xl font-display font-bold mb-6 text-foreground section-rail flex items-center">
              Featured Games
            </h2>
          ...
          {Object.entries(categories).map(([category, categoryGames]) => (
            <section key={category} id={category.toLowerCase().replace(/\s+/g, '-')} className="scroll-mt-20">
              <h2 className="text-2xl font-display font-bold mb-6 text-foreground section-rail flex items-center">
